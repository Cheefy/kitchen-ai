...
- **`activity_log`** — `id`, `date`, exercise data (see Garmin integration, Section 9), `garmin_activity_id` (nullable, unique — dedupes repeated syncs of overlapping date ranges; missing from the original field list until the sync was actually built).
