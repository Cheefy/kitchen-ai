...
**Compose services** (`ollama` + `backend` only):
- **`backend`** — the FastAPI app, built from a local Dockerfile. Talks to Postgres over the LAN (see above) and to Ollama over the internal Compose network (`ollama:11434`).
- **`ollama`** — official `ollama/ollama` image, serves the tool-calling command-routing LLM from §1. Named volume for `/root/.ollama` so pulled model weights persist across restarts/rebuilds.
  - GPU passthrough: NVIDIA (RTX 4000 Ada, RTX 4000 Blackwell) is the straightforward path via `nvidia-container-toolkit` + Compose `deploy.resources.reservations.devices` (or `--gpus all`) — Ollama's primary supported Docker route. Arc Pro B70 (Intel) GPU passthrough into a Docker container is comparatively immature (typically needs IPEX-LLM builds or the Vulkan backend, not turnkey) — treat the two NVIDIA cards as the v1 inference GPUs and revisit Arc support later rather than blocking on it.
  - Model choice needs tool/function-calling support and enough context for a session's recipe + inventory state — benchmark plan, not yet committed, see §12/§14.
- Only `backend`'s API/web port (8000) is published to the LAN, for the iPad PWA and the satellite device. Ollama stays internal-only on the Compose network.
- **Remote access**: Tailscale runs on the Docker host itself, not as a container — it reaches any published port (backend's 8000, Postgres's 5433) like any other LAN service, so no sidecar/ACL setup is needed for v1.
...
- `ollama` has a ready-made Community Applications (CA) template too, but stays in the Compose stack here for simplicity (one `docker compose up` for the two services that actually change together). `backend` is custom code — not something CA has; it needs its own `Dockerfile`, built into an image and run as one service in the Compose stack.
- Cleanest path for the two-service stack: the **Docker Compose Manager** CA plugin, which takes one `docker-compose.yml` pasted into the Unraid UI, rather than wiring up a separate Docker template by hand.
- **GPU passthrough is not automatic on Unraid** — the **Nvidia-Driver** CA plugin (community, ich777) has to be installed at the OS level before any container (Ollama included) can see the GPUs via `--runtime=nvidia` / Compose GPU reservations. A comparable Intel GPU plugin exists from the same author for the Arc card, but the LLM-inference support gap noted above is independent of Unraid.
- Persistent volumes (Ollama model cache in the Compose stack; Postgres's own data dir in its separate CA container) live under `/mnt/apps/appdata/<container>/` — this box keeps Docker appdata on its own dedicated pool named `apps`, not Unraid's usual default `/mnt/user/appdata/`.
...
### Tracking
- **`meal_log`** — one row per thing eaten. `id`, `timestamp`, `recipe_id` / `meal_prep_batch_id` / `ingredient_id` (whichever applies, nullable), `quantity`, and macros **snapshotted at log time** (editing a recipe later doesn't retroactively change past logged meals). `is_estimate` (bool) — see Section 7 for the toggle behavior. All fields editable after the fact on the iPad.
...
- **Ollama model choice** — benchmark plan defined (§12); Qwen3 is the leading candidate but not committed until tested against realistic compound-command phrasing.
