...
**Compose services**:
- **`postgres`** — official `postgres` image, named volume for `/var/lib/postgresql/data`, healthcheck (`pg_isready`) gates backend startup.
- **`backend`** — the FastAPI app, built from a local Dockerfile, `depends_on: postgres (healthy)`. Talks to Postgres and Ollama over the internal Compose network (`postgres:5432`, `ollama:11434`) — neither is published to the host.
...
- Only the backend's API/web port is published to the LAN (for the iPad PWA and the satellite device); Postgres and Ollama stay internal-only on the Compose network.
- **Remote access**: Tailscale runs on the Docker host itself, not as a container — it reaches the published backend port like any other LAN service, so no sidecar/ACL setup is needed for v1.
...
- `postgres` and `ollama` both have ready-made Community Applications (CA) templates and can be pulled from there directly. `backend` is custom code (this project's FastAPI app) — not something CA has; it needs its own `Dockerfile`, built into an image and run either as a manual CA template or as one service in a Compose stack.
- Cleanest path for a multi-service stack: the **Docker Compose Manager** CA plugin, which takes one `docker-compose.yml` (all services) pasted into the Unraid UI, rather than wiring up separate Docker templates by hand.
...
- Persistent volumes (Postgres data dir, Ollama model cache) follow Unraid's `/mnt/user/appdata/<container>/` convention.
